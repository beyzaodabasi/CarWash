...
const { store, login, smsLogin, index, show, update, destroy } = require('../controllers/MemberController')
...
router.route('/smslogin').post(checkVersion, validate(schemas.smsLoginValidation), smsLogin)
router.route('/').get(authenticateToken, checkVersion, index)
router.route('/').post(checkVersion, validate(schemas.createValidation), checkAge, store)
